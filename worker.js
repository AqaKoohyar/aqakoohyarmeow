const _D_={_vl_:atob('dmxlc3M='),_tr_:atob('dHJvamFu'),_vm_:atob('dm1lc3M='),_ss_:atob('c2hhZG93c29ja3M='),_wg_:atob('d2lyZWd1YXJk'),_cl_:atob('Y2xhc2g='),_sb_:atob('c2luZ2JveA=='),_sb2_:atob('c2luZy1ib3g='),_mh_:atob('bWlob21v'),_hd_:atob('aGlkZGlmeQ=='),_sg_:atob('c3VyZ2U='),_qx_:atob('cXVhbng='),_ln_:atob('bG9vbg=='),_np_:atob('bWVvd1Byb3h5'),_np2_:atob('bWVvdy1Qcm94eQ=='),_np3_:atob('bWVvdw=='),_cf_:atob('Y2xvdWRmbGFyZQ=='),_xr_:atob('eHJheQ=='),_cr_:atob('Q21saXVzcw=='),_pr_:atob('UFJPWFlJUA=='),_sp_:atob('c3BlZWQuY2xvdWRmbGFyZS5jb20='),_wr_:atob('bWVvdy1XQVJQ'),_ws_:atob('d3M='),_grpc_:atob('Z3JwYw=='),_xhttp_:atob('eHR0cA=='),_aes128_:atob('YWVzLTEyOC1nY20='),_aes256_:atob('YWVzLTI1Ni1nY20='),_chrome_:atob('Y2hyb21l'),_mixed_:atob('bWl4ZWQ=')};const Version = 'V4.0.0';
...
const Pages静态页面 = 'https://aqakoohyar.github.io/';
const meow_FREE_NOTICE = '🎁 میو رایگان است، برای آن به کسی پول ندهید';
...
async function 获取meow版本() {
	for (const u of [meow版本URL, meow仓库RAW + '/version.json']) {
		try { const r = await fetch(u, { headers: { 'User-Agent': 'meowProxy' }, cf: { cacheTtl: 0 } }); if (r.ok) { const j = await r.json(); if (j && j.version) return j; } } catch (e) {}
	}
	return null;
}
///////////////////////////////////////////////////////查杀特征码///////////////////////////////////////////////
...
function panelUnavailableHtml() {
	return '<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>meow Proxy — setup</title>'
...
function wrapKVWithD1(env) {
	...
	env.KV = {
		...
		put: async (k, v, opts) => {
			...
			if (realKV && (env.meow_KV_MIRROR === '1' || env.meow_KV_MIRROR === 'true')) { try { realKV.put(k, v, opts).catch(() => {}); } catch (e) {} }
		},
		delete: async (k) => {
			try { if (await d1Init(env)) await env.DB.prepare('DELETE FROM kvstore WHERE k=?').bind(k).run(); } catch (e) {}
			if (realKV && (env.meow_KV_MIRROR === '1' || env.meow_KV_MIRROR === 'true')) { try { realKV.delete(k).catch(() => {}); } catch (e) {} }
		},
		list: async (opts) => {
			opts = opts || {};
			try {
...
async function 服务用户中心() {
	try {
		...
		const r = await fetch(base + '/user/index.html', { headers: { 'User-Agent': 'meowProxy' }, cf: { cacheTtl: 300, cacheEverything: true } });
...
async function 获取Nat64前缀() {
	...
	if (/^https?:\/\//i.test(src)) {
		...
		try {
			const r = await fetch(src, { headers: { 'User-Agent': 'meowProxy' } }); const txt = await r.text();
...
export default {
	async fetch(request, env, ctx) { try {
		...
		const 命中XHTTP特征 = referer.includes('x_padding', 14) || referer.includes('x_padding=');
			...
			// 后端模式诊断：在浏览器访问 /backend-test 查看 meow 连接后端时的实际情况
			...
			if (env.KV && typeof env.KV.get === 'function') {
				...
				if (访问路径 === 'admin/security/status') {
						...
						const label = encodeURIComponent('meow Proxy (' + url.host + ')');
						const otpauth = `otpauth://totp/${label}?secret=${secret}&issuer=${encodeURIComponent('meow Proxy')}&algorithm=SHA1&digits=6&period=30`;
				...
				if (访问路径 === 'admin/init') {// 重置配置为默认值
						...
						if (request.method === 'POST') {
							...
							try {
								if (reqBody.fromCentral) {
									...
									let keys = []; try { const cr = await fetch(api + '/api/warp', { headers: { 'User-Agent': 'meowProxy' } }); const cj = await cr.json(); keys = Array.isArray(cj.keys) ? cj.keys : []; } catch (e) {}
						...
						let 源码地址 = meowWorkerSrcFallback, 最新版本 = '';
						{ const 版本信息 = await 获取meow版本(); if (版本信息) { if (版本信息.worker_url) 源码地址 = 版本信息.worker_url; 最新版本 = String(版本信息.version || '').replace(/^[vV]/, ''); } }
						let 脚本文本 = '';
						try { const r = await fetch(源码地址, { headers: { 'User-Agent': 'meowProxy' } }); if (!r.ok) throw new Error('HTTP ' + r.status); 脚本文本 = await r.text(); } catch (e) { return _更新错误响应('下载Worker源码失败', { detail: (e && e.message) || String(e) }); }
						...
						try {
							const r = await fetch(`${url.protocol}//${url.host}/sub?token=${encodeURIComponent(tok)}&singbox`, { headers: { 'User-Agent': 'sing-box/1.11.0 meow-debug' } });
					...
					const 订阅TOKEN = await MD5MD5(host + userID), 作为优选订阅生成器 = ['1', 'true'].includes(env.BEST_SUB) && url.searchParams.get('host') === 'example.com' && url.searchParams.get('uuid') === '00000000-0000-4000-8000-000000000000' && UA.toLowerCase().includes('tunnel (https://github.com/' + 特征码字典[1] + '/meow');
				...
				const 用户客户端请求订阅 = 请求TOKEN === 订阅TOKEN || 订阅用户Tag !== '';
					...
					if (用户客户端请求订阅 || 订阅转换后端请求订阅 || 作为优选订阅生成器) {
						...
						try {
							...
							const _profileName = (config_JSON.优选订阅生成.SUBNAME && config_JSON.优选订阅生成.SUBNAME !== 'meow Proxy')
								? config_JSON.优选订阅生成.SUBNAME : '🎁 سرویس رایگان میو';
						...
						if (订阅类型 === 'mixed') {
							...
							完整优选IP = [host + ':443#' + meow_FREE_NOTICE, ...完整优选IP];
							...
							订阅内容 = 其他节点LINK + 完整优选IP.map((原始地址, _idx) => {
								...
								// Node remark: Persian free-service brand + number (+ protocol tag in mixed mode)
								{
									...
									节点备注 = 'سرویس رایگان میو ' + (_idx + 1) + _protoMark + _chainMark;
								}
								if (_np === 'ss' && !作为优选订阅生成器) {
							...
							try {
								const response = await fetch(订阅转换URL, { headers: { 'User-Agent': 'Subconverter for ' + 订阅类型 + ' edge' + 'tunnel (https://github.com/' + 特征码字典[1] + '/meow' + 'Proxy)' } });
								if (response.ok) {
		...
		try { console.error('meow fatal:', (topLevelError && (topLevelError.stack || topLevelError.message)) || String(topLevelError)); } catch (e) {}
		try {
...
function _warpPublicView(a, epOverride) {
	...
	if (a.privateKey && a.peerPublicKey) {
		...
		v.node = `wireguard://${encodeURIComponent(a.privateKey)}@${epFull}/?publickey=${encodeURIComponent(a.peerPublicKey)}${reservedStr}&address=${encodeURIComponent(addr)}&mtu=1280#meow-WARP`;
...
///////////////////////////////////////////////////////中央服务器管理钩子///////////////////////////////////////////////
// 选择性启用：设置 CENTRAL_API（环境变量）或 centralApi（config.json中的centralApi字段）为meow控制面板地址。
...
async function 中央心跳(env) {
	...
	const id = await MD5MD5('meow-instance:' + host); // 稳定、不可逆的实例ID
	let usage = null; try { usage = await usageGet(env, 'usage-m:' + (new Date().getFullYear() + '-' + String(new Date().getMonth() + 1).padStart(2, '0'))); } catch (e) { }
	try {
		await fetch(api + '/heartbeat', { method: 'POST', headers: { 'Content-Type': 'application/json', 'User-Agent': 'meowProxy' }, body: JSON.stringify({ id, host, version: Version, monthTraffic: usage ? usage.total : 0, ts: Date.now() }) });
	} catch (e) { /* best-effort */ }
}
async function 刷新公告(env) {
	const { api } = await 获取CentralApi(env); if (!api) return;
	try {
		const r = await fetch(api + '/announcement', { headers: { 'User-Agent': 'meowProxy' } });
...
function buildWarpWireGuardLink(ipPort, group, mtu) {
	...
	const remarks = encodeURIComponent('meow-WARP-' + ipPort);
...
function buildWarpNekoRayLink(ipPort, group, mtu) {
	...
	const cfg = { _v: 0, addr: '127.0.0.1', cmd: [''], core: 'internal', cs, mapping_port: 0, name: 'meow-WARP-' + ipPort, port: 1080, socks_port: 0 };
...
async function buildRegisteredWarpNode(env) {
	...
	return `wireguard://${encPriv}@${ep.includes(':') ? ep : ep + ':2408'}/?publickey=${encPub}${reservedStr}&address=${addr}&mtu=1280#meow-WARP`;
}
...
`;
	...
	if (网络设置 && 网络设置.enableWarp && warpAccount && warpAccount.registered && warpAccount.privateKey && !clash_yaml.includes('name: "meow-WARP"')) {
		try {
			...
			if (网络设置.warpMode === 'wow' && warpAccount.wow && warpAccount.wow.privateKey) {
				block = wgProxy(warpAccount, 'meow-WARP', epOverride || warpAccount.endpoint, '') + '\n' +
					wgProxy(warpAccount.wow, 'meow-WoW', '162.159.192.1:2408', 'meow-WARP');
				matchTarget = 'meow-WoW';
			} else {
				let dialer = '';
				...
				block = wgProxy(warpAccount, 'meow-WARP', epOverride || warpAccount.endpoint, dialer);
				matchTarget = 'meow-WARP';
			}
		...
		if (网络设置 && 网络设置.enableWarp && warpAccount && warpAccount.registered && warpAccount.privateKey) {
			...
			if (!config.outbounds.some(o => o && o.tag === warpTag)) {
				...
				if (网络设置.warpMode === 'wow' && warpAccount.wow && warpAccount.wow.privateKey) {
					...
					config.outbounds.push(buildWarpSingboxOutbound(warpAccount.wow, 'meow-WoW', warpTag, null, true));
					确保Route().final = 'meow-WoW';
				} else {
					let detourTag = null;
					if (网络设置.warpMode === 'chain') {
...
async function 获取池文件(fileUrl) {
	...
	let list = [];
	try {
		const r = await fetch(fileUrl, { headers: { 'User-Agent': 'meowProxy' }, cf: { cacheTtl: 1800, cacheEverything: true } });
...
async function 获取智能清洁IP(request, poolApi, count) {
	...
	for (const f of [...new Set([carrier, 'ir', 'all'])]) {
		...
		if (list && list.length) {
			...
			return shuffled.map(line => line.includes('#') ? line : (line + '#meow-' + f.toUpperCase()));
		}
	}
	return [];
}
...
async function 检查域名健康(env, hosts, selfHost) {
	...
	await Promise.all(checkable.map(async host => {
		...
		try {
			const opts = { headers: { 'User-Agent': 'meowHealth/1.0' } };
...
async function 获取优选订阅生成器数据(优选订阅生成器HOST) {
	...
	try {
		const response = await fetch(优选订阅生成器URL, {
			headers: { 'User-Agent': 'v2rayN/meow' + 'tunnel (https://github.com/' + 特征码字典[1] + '/meow' + 'Proxy)' }
		});
...
function 主菜单文本() {
	return `<b>🛰 به ربات meow Proxy خوش آمدید</b>
...
async function 处理TelegramWebhook(request, env, userID, host, encryptionKey = null) {
	try {
		...
		switch (cmd) {
			...
			case '/help': {
				const helpText = `<b>╔═══❰✨ meow Proxy Bot ❱═══╗</b>
...
<b>╚════════════════╝</b>`;
			...
			case '/install': {
				const scriptName = (args || 'aqakoohyar').replace(/[^a-zA-Z0-9_-]/g, '').slice(0, 50) || 'aqakoohyar';
...
async function runCfInstall(env, botToken, chatId, host, request) {
	...
	const scriptName = st.scriptName || 'aqakoohyar';
...
async function announceSubLinks(env, opts = {}) {
	try {
		...
		const lines = ['<b>🔥 لینک‌های اشتراک meow / meow subscription links</b>', ''];
		if (baseUrl) {
...
async function publishSubMirror(env, baseUrl) {
	try {
		...
		for (const f of files) {
			try {
				const r = await fetch(`${baseUrl}/sub?token=${await MD5MD5(baseUrl.replace(/^https?:\/\//, '') + (env.UUID || ''))}&${f.q}`, { headers: { 'User-Agent': 'meowMirror/1.0' } });
				...
				const existing = await fetch(putUrl, { headers: { 'Authorization': 'token ' + gh.token, 'User-Agent': 'meowMirror' } });
				const existingJson = await existing.json().catch(() => ({}));
				const sha = existingJson && existingJson.sha ? existingJson.sha : undefined;
				const body = { message: `meow: update ${f.name}`, content: btoa(unescape(encodeURIComponent(content))), branch: gh.branch || 'main' };
				if (sha) body.sha = sha;
				const put = await fetch(putUrl, { method: 'PUT', headers: { 'Authorization': 'token ' + gh.token, 'Content-Type': 'application/json', 'User-Agent': 'meowMirror' }, body: JSON.stringify(body) });
...
//////////////////////////////////////////////////////meow Radar/Scan Page///////////////////////////////////////////////
function meowScanPage() {
	const html = `<!DOCTYPE html><html lang="fa" dir="rtl" data-theme="dark"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>meow Radar</title><style>
...
<header class="topbar">
<div class="brand"><div><div class="name">meow Radar</div><div class="env"><span class="d"></span>clean-IP scanner</div></div></div>
...
<script>
var lang=(function(){try{var q=new URLSearchParams(location.search).get('lang');if(q==='en'||q==='fa')return q;return localStorage.getItem('meow-user-lang')==='en'?'en':'fa';}catch(e){return 'fa';}})();
var theme=(function(){try{return localStorage.getItem('meow-theme')==='light'?'light':'dark';}catch(e){return 'dark';}})();
...
function buildConfig(ip,port){if(!TMPL)return null;port=port||TMPL.port||443;return 'vless://'+TMPL.uuid+'@'+ip+':'+port+'?'+TMPL.query+'#'+encodeURIComponent('meow ⚡ '+ip+':'+port);}
async function run(){
...
$('lang').addEventListener('click',function(e){var b=e.target.closest('button');if(!b)return;lang=b.dataset.l;try{localStorage.setItem('meow-user-lang',lang);}catch(_){}applyLang();try{loadTemplate();}catch(_e){}});
$('theme').addEventListener('click',function(e){var b=e.target.closest('button');if(!b)return;theme=b.dataset.t;try{localStorage.setItem('meow-theme',theme);}catch(_){}applyTheme();});
...
// meow 内部路径：即使在后端模式下也绝不允许转发到后端（DoH、面板、登录、机器人、安装、WARP/订阅生成器等）
function 是否后端排除路径(访问路径, pathname) {
	...
	if (p === 'login' || p === 'bot' || p === 'setwebhook' || p === 'version' || p === 'meow-block' || p === 'locations' || p === 'robots.txt') return true;
...
// 转发非升级请求（xhttp/gRPC POST 流）到后端，请求体直接透传，后端处理协议，meow 转发响应
...
// 后端模式诊断：在浏览器访问 /backend-test，报告后端模式是否开启、后端地址，以及 meow 真正尝试连接后端时的结果
async function 后端诊断(env, url) {
	...
	let target = '';
	try { const b = new URL(_bm.url); if (b.pathname === '/' || !b.pathname) b.pathname = '/meowvpn'; target = b.toString(); } catch (e) { out.steps.push('Backend URL is not a valid URL: ' + (e && e.message)); return new Response(JSON.stringify(out, null, 2), { status: 200, headers: { 'Content-Type': 'application/json;charset=utf-8' } }); }
	...
	try {
		...
		if (r.status === 101 && r.webSocket) {
			out.ok = true;
			out.steps.push('SUCCESS: meow reached your backend and it upgraded to WebSocket (101). The relay path works. If a client still fails, the issue is client-side (UUID/path/TLS in the link), not the backend.');
		...
		out.steps.push('meow could NOT reach the backend at all (fetch threw). Cloudflare Workers cannot open an outbound connection to a raw http:// IP on a non-standard port. Front your backend with TLS on 443/2053/2083/2087/2096/8443 via a (sub)domain.');
